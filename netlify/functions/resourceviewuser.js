...
exports.handler = async(event, context) => {
    ...
    {    
        ...
        if(querytable === 0)
        {
            query = 'SELECT id,name,link,categories,pricing,platforms,tags,description,status,date FROM user_submitted WHERE status=$1 ORDER BY id ASC';
        }
        if(querytable === 1)
        {
            query = `SELECT * FROM user_submitted WHERE status='Accepted' ORDER BY id ASC`;
        }
        if(querytable === 2)
        {
            query = `SELECT * FROM user_submitted WHERE status='Rejected' ORDER BY id ASC`;
        }
        else
        {
            query = `SELECT * FROM user_submitted'+wherestring+'ORDER BY id ASC`;
        }
